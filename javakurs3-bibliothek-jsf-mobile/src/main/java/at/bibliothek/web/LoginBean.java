...
//				request.getSession().setAttribute(SESSION_ATTRIBUTE_USERID, new Integer(user.getId()));
				...
				Logger.getLogger(NewCustomerBean.class.getName()).error("An exception occurec while trying to log user " + " this.getLogin(): " + e.getMessage(), e);
				Logger.getLogger(NewCustomerBean.class.getName()).warn("Now, trying to logout the user " + " this.getLogin(): ");
	...
	private static void loginUserAndSetUserIdSessionAttribute(HttpServletRequest request, String username, String password, User user) throws ServletException {
		request.login(username, password);
		HttpSession session = request.getSession(true);
		
		session.setAttribute(SESSION_ATTRIBUTE_USERID, new Long(user.getId()));
	}
