...
public class NewCustomerBean implements Serializable {
	...
	public void init() {
		...
		custmerTypes.put(KundeTyp.TYP_NATPERS.toString(), KundeTyp.TYP_NATPERS);
		custmerTypes.put(KundeTyp.TYP_FIRMA.toString(), KundeTyp.TYP_FIRMA);
	}
	...
	public String saveNewCustomer(ActionEvent e) {
		...
		case TYP_FIRMA: {
			...
			((Firma) neuerKunde).setSteuerNummer(this.getSteuerNummer());
			break;
		}
		}
		...
		try {
			...
			Logger.getLogger(NewCustomerBean.class.getName()).error(se.getMessage(), se);
